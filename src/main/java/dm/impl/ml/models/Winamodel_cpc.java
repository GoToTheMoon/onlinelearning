...
public class Winamodel_cpc extends WinaBaseModel implements WinaInterface {
    private static Winamodel_cpc instance;
    private ReentrantReadWriteLock rwl = new ReentrantReadWriteLock();
    double recognition=0.02;
    double dsp_cpc=0.5;

    public static HashMap<String,Double> ad_price_1=new HashMap<>();
    public static HashMap<String,Double> ad_compitition=new HashMap<>();

    public static synchronized Winamodel_cpc getInstance(String file_path,HashMap<String,HashMap<String,List>> id) throws IOException {
        if(instance==null)
            instance = new Winamodel_cpc(id);
        return instance;
    }

    public static synchronized Winamodel_cpc getInstance(String file_path, String mode) throws Exception {
        if(instance==null)
            instance = new Winamodel_cpc(mode);
        return instance;
    }

    public Winamodel_cpc(HashMap<String,HashMap<String,List>> id) throws IOException{
    ...
    public Winamodel_cpc(String mode) throws IOException {
        super(mode);
        ...
        if (rwl.isWriteLocked()){
            ...
            for (int i = 0; i < adlist.length(); i++) {
                ...
                if (htt[0].contains("cpro")) {
                    ...
                    pricesum += price * compit * recognition;
                    prices[i] = pricesum;
                    price_aid.put(pricesum, adid);
                    p = 2 * price * compit * recognition;

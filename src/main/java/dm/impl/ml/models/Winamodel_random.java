...
public class Winamodel_random extends WinaBaseModel implements WinaInterface{
    private static Winamodel_random instance;

    public static synchronized Winamodel_random getInstance(String file_path,HashMap<String,HashMap<String,List>> id) throws Exception {
        if(instance==null)
            instance = new Winamodel_random(id);
        return instance;
    }

    public static synchronized Winamodel_random getInstance(String file_path,String mode) throws Exception {
        if(instance==null)
            instance = new Winamodel_random(mode);
        return instance;
    }

    public Winamodel_random(HashMap<String,HashMap<String,List>> id) throws IOException {
        imeikeyword=id.get("imeikeyword");
        androidkeyword=id.get("androidkeyword");
    }

    public Winamodel_random(String mode) throws IOException {
        super(mode);
    }
